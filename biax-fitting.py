...
def residual(c,c_all,c_fix,measure,inp,sample,_dsampledtheta,_LOGNORM=False,_FINITE_DIFFERENCE=False):
...
def CentralDifference(c,c_all,c_fix,measure,inp,sample,_dsampledtheta,_LOGNORM=False,_FINITE_DIFFERENCE=False):
    ...
    dres = residual(list(c_all_up.values())[3:],c_all_up,c_fix,measure,inp,sample,_dsampledtheta,_LOGNORM,_FINITE_DIFFERENCE) - residual(list(c_all_down.values())[3:],c_all_down,c_fix,measure,_dsampledtheta,_LOGNORM,_FINITE_DIFFERENCE)
    step = c_all[c]*0.002
    return dres/step
    

def Dresidual(c,c_all,c_fix,measure,inp,sample,_dsampledtheta,_LOGNORM=False,_FINITE_DIFFERENCE=False):
    '''
    d(residual)/d(c_all)
    Can be defined analytically or approximately
    '''
    # pool = Pool(2) # mp.cpu_count()
    complete_params(c,c_all,c_fix)
    if _LOGNORM == False:
        if _FINITE_DIFFERENCE == True:
            res = residual(c,c_all,c_fix,measure,inp,sample,_dsampledtheta,_LOGNORM,_FINITE_DIFFERENCE)
            length = len([c_var for c_var in c_all if (c_fix[c_var] == False)])
            with Pool(processes) as pool:
                return np.array(pool.starmap(ForwardDifference,zip([c_var for c_var in c_all if (c_fix[c_var] == False)],\
                                                                np.tile([c_all],length),np.tile([c_fix],length),\
                                                                np.tile(res,(length,1)),np.tile(inp,(length,1,1)),\
                                                                np.tile(measure,(length,1,1)),np.tile(sample,length),\
                                                                np.tile([_dsampledtheta],length),\
                                                                np.tile([_LOGNORM],length),\
                                                                np.tile([_FINITE_DIFFERENCE],length)))).transpose()
        elif _FINITE_DIFFERENCE == False:
            return np.array([dsampledtheta[c_var[:-2]].disp_controlled(inp,c_all) for c_var in c_all if (c_fix[c_var] == False)]).transpose()
    elif _LOGNORM == True:
        x = sample.disp_controlled(inp,c_all)
        # orig_len = len(measure)
        res = residual(c,c_all,c_fix,measure,inp,sample,_dsampledtheta,_LOGNORM,_FINITE_DIFFERENCE)
        if _FINITE_DIFFERENCE == True:
            # return np.array([ForwardDifference(c_var,\
            #                           c_all,\
            #                           c_fix,\
            #                           res.flatten(),\
            #                           inp,\
            ...
            #                           _dsampledtheta,\
            #                           _LOGNORM,\
            #                           _FINITE_DIFFERENCE) for c_var in c_all if (c_fix[c_var] == False)]).transpose()
            length = len([c_var for c_var in c_all if (c_fix[c_var] == False)])
            with Pool(processes) as pool:
                return np.array(pool.starmap(ForwardDifference,zip([c_var for c_var in c_all if (c_fix[c_var] == False)],\
                                                                np.tile([c_all],length),np.tile([c_fix],length),\
...
if __name__ == '__main__':
    ...
    if mat=='yeoh':
        ...
        arb_mat = ARB(W_string[:-3],init_guess_string[:-2],lower_bound_string[:-2],upper_bound_string[:-2], dparams=False)
